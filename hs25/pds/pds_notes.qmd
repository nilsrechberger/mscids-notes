...
Python enables to structure project resources using packages. a folder becomes a package as soon as it comprises
a script named `__init__.py`
...
The Python Software Foundation hosts an own repository for official Python packages called [PyPI](pypi.org). `pip` is a command-line interface (CLI) tool for installing Python packages.
...
- Rich Text Support: include [Markdown](https://www.markdownguide.org/) for formatting text, including headings, lists, links, and images.
...
Python only has objects and references to them. So, variables are always references to objects stored in the memory in one
or more storage cells. Consequently, when copying a variable, we copy the reference to a particular object – not the object itself.
...
- Iterable: An object that you can use in a for loop (e.g., list, string, dictionary). It has the __iter__ method.
- Iterator: The object that actually does the work. It has the __next__ method and returns the values to you one after the other.
