...
p_annual <- p_e2 + p_e3 - intersection
...
cat(A_only, "or", B_only)
```

### d

```{r}
opposite_intersection <- -(A + B)
print(opposite_intersection)
```
...
only_earthquake <- p_earthquake - p_typhoon
only_typhoon <- p_typhoon - p_earthquake

print(only_earthquake)
print(only_typhoon)
```