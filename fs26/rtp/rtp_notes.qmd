...
```{r}
# Fit a separate fitting parameter for each state
fatal_fe_lm_mod <- lm(fatal_rate ~ beertax + state - 1,
data = Fatalities)
summary(fatal_fe_lm_mod)

# Subtract time averaged panel data
...
predictions <- predict(probitModelEthn, newdata = data.frame("a" = c("no", "yes"), "pirat" = c(0.3, 0.3)), type = "response")
