...
**1. Total Variables (p)**
There are 6 variables in total: Prefix, Assignment, Tutorial, Midterm, TakeHome, and Final.
* `p = 6`

**2. Observed Variables per Pattern (p_j)**
Based on the `md.pattern` output, we have three distinct missingness patterns:
* **Pattern 1 (Complete cases):** All 6 variables are fully observed for 95 rows.
    * $p_1 = 6$
* **Pattern 2 (Missing Final):** 5 variables are observed (Final is missing) for 3 rows.
    * $p_2 = 5$
* **Pattern 3 (Missing TakeHome):** 5 variables are observed (TakeHome is missing) for 1 row.
    * $p_3 = 5$
...
> Note: [Shakeel et. al. (2020)](https://dl.acm.org/doi/10.1145/3356901)
