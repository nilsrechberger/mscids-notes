...
![Randomized Controlled Tree](img/random_tree.png){width=50%}
...
![Decision Tree for Research Design](img/decision_tree_for_research_design.png){width=50%}
...
![Landscape of empirical research](img/landscape_of_empirical_research.png){width=50%}
...
![Summary Variance](img/var_summary.png){width=50%}
...
![Relationship between Internal vs. External Validity](img/internal_vs_external_validity.png){width=50%}
...
![Sampling procedure Overview](img/sampling_map.png){width=50%}
...
Effect size is a statistical measure that quantifies the magnitude (or strength) of a phenomenon. A tiny p-value in a very large study might show a difference that is statistically significant, but practically meaningless. Effect size expresses zhe difference in terms of standard deviations.
...
![Effect Size Table](img/effect_size_table.png){width=60%}
...
![Profile Plots of Main Effect](img/profile_plot_main_effect.png){width=30%}
...
![Profile Plot of Interaction Effect](img/profile_plot_interaction_effect.png){width=30%}
...
![Overview over Statistical Hypothesis Tests](img/hyp_test_overview.png){width=50%}
...
![Latin Square](img/latin_square.png){width=50%}
...
Measure for the size of the bias → Mean-squared error (MSE). The MSE measures the deviation (bias) of the estimator $\bar{x}$ from the mean $\mu_0$ in the population. The bias goes to $0$ only, if the size $n$ of the administrative dataset goes against $N$ ($n$ → $N$). The absolute size $n$ of the administrative dataset is meaningless without specifying $N$.
...
library(readxl)
data <- read_excel(path="./data/clickrate.xlsx")
...
he column names of the Latin square ([,1] [,2] [,3]) correspond to the levels of factor A.
...
```{r}
library(readxl)
dwelltime_Latin <- read_excel("./data/dwelltime_latin.xlsx")
